...
def initdb_command():
    ...
    if os.path.isfile(DATABASE):
        print("Database already exists, aborting: {}".format(DATABASE), file=sys.stderr)
    else:
        init_db()
...
# COMPLICATED_URL_REGEX = re.compile(ur'(?i)\b((?:https?://|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:\'".,<>?\xab\xbb\u201c\u201d\u2018\u2019]))')
...
def imgify(tweet):
    """Given the tweet string, returns an HTML version with the image
       URLs marked up into fancy <img> tags
